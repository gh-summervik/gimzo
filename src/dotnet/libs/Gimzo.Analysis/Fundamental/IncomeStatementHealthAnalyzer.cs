...
public static class IncomeStatementHealthAnalyzer
{
    ...
    private static double NormalizeRoic(double value)
        => value >= 0.20 ? 1.0 :
           value >= 0.15 ? 0.85 :
           value >= 0.10 ? 0.65 :
           value >= 0.05 ? 0.4 :
    ...
    public static IncomeStatementAssessment Assess(ProfitabilityRatios profitability, EfficiencyRatios efficiency)
    {
        ...
        double? roicProxy = profitability.OperatingMargin.HasValue && efficiency.AssetTurnoverRatio.HasValue
                    ? profitability.OperatingMargin.Value * efficiency.AssetTurnoverRatio.Value
                    : null;

        double sRoic = roicProxy.HasValue ? NormalizeRoic(roicProxy.Value) : 0.0;

        double composite = (sGross + sOp + sProfit + sRoe + sRoa + sRoic) / 6.0;
        ...
        return new IncomeStatementAssessment(score, assessment,
                    profitability.GrossMargin, profitability.OperatingMargin, profitability.ProfitMargin,
                    profitability.ReturnOnEquity, profitability.ReturnOnAssets, roicProxy);
    }
}