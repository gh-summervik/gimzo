    ...
    ILogger<FinancialDataImporter> logger) : ServiceBase(dbDefPair, memoryCache)
{
    ...
    internal async Task DoWeekdayWorkAsync()
    {
        ...
        var stockSymbolModels = (await _fdnApiClient.GetStockSymbolsAsync())
            .Where(k => !string.IsNullOrWhiteSpace(k.Symbol) &&
                    !_metaInfo.IgnoredSymbols.Contains(k.Symbol));
    ...
    internal async Task DoSaturdayWorkAsync()
    {
        ...
        foreach (var symbol in symbols)
        {
            ...
            if (incStmtsModels.Length > 0)
                await SaveIncomeStatementsAsync([.. incStmtsModels.Select(k => k.ToDomain())], _process!.ProcessId);

            var bsStmtsModels = await _fdnApiClient.GetBalanceSheetStatementsAsync(symbol);
            if (bsStmtsModels.Length > 0)
                await SaveBalanceSheetsAsync([.. bsStmtsModels.Select(k => k.ToDomain())], _process!.ProcessId);

            var cfStmtsModels = await _fdnApiClient.GetCashFlowStatementsAsync(symbol);
            if (cfStmtsModels.Length > 0)
            {
                IReadOnlyCollection<Gimzo.Analysis.Fundamental.CashFlowStatement> statements = [.. cfStmtsModels.Select(k => k.ToDomain())];
                await SaveCashFlowStatementsAsync(statements, _process!.ProcessId);
            }

            var divsModels = await _fdnApiClient.GetDividendsAsync(symbol);
            if (divsModels.Length > 0)
                await SaveDividendsAsync([.. divsModels.Select(k => k.ToDomain())], _process!.ProcessId);

            var splitsModels = await _fdnApiClient.GetStockSplitsAsync(symbol);
            if (splitsModels.Length > 0)
            {
                IReadOnlyCollection<Gimzo.Analysis.Fundamental.StockSplit> splits = [.. splitsModels.Select(k => k.ToDomain())];
                await SaveSplitsAsync(splits, _process!.ProcessId);
            }

            var earningsReleasesModels = await _fdnApiClient.GetEarningsReleasesAsync(symbol);
            if (earningsReleasesModels.Length > 0)
                await SaveEarningReleasesAsync([.. earningsReleasesModels.Select(k => k.ToDomain())], _process!.ProcessId);

            var shortsModels = await _fdnApiClient.GetShortInterestAsync(symbol);
            if (shortsModels.Length > 0)
                await SaveShortInterestsAsync([.. shortsModels.Select(k => k.ToDomain())], _process!.ProcessId);
        }
    }
...
WHERE p.symbol IS NULL";
        ...
        symbols = [.. (await queryCtx.QueryAsync<string>(FindEmptyPricesSql))];
        if (symbols.Length > 0)
...
HAVING COUNT(*) < @MinDays";
    ...
    public async Task UpdateSicTitlesAsync()
    {
        ...
        const string UpdateTitlesSql = @"UPDATE public.us_companies
SET sic_title = @Title,
updated_by = @User,
updated_at = @Now,
...
";
        ...
        foreach (var code in Constants.SicTitles.Keys)
        {
            await cmdCtx.ExecuteAsync(UpdateTitlesSql, new
            {
                Title = Constants.SicTitles[code],
                User = _process!.ProcessId,
                Now = now,
                UnixMs = unixMs,
